      ...
      {screen === 'registration' && (
        <div className="max-w-md mx-auto bg-white p-8 rounded-2xl shadow-xl border border-slate-100">
          ...
          <form onSubmit={handleStart} className="space-y-4">
            <input required placeholder="Nombre Completo" className="w-full p-3 border rounded-lg" onChange={e => setUserData({...userData, name: e.target.value})} />
            <input required placeholder="Cargo" className="w-full p-3 border rounded-lg" onChange={e => setUserData({...userData, role: e.target.value})} />
            <input required placeholder="Sucursal" className="w-full p-3 border rounded-lg" onChange={e => setUserData({...userData, store: e.target.value})} />
        ...
        <div className="max-w-2xl mx-auto">
          ...
          {QUESTIONS.filter(q => q.level === currentLevel).map((q) => (
            <div key={q.id} className="bg-white p-6 rounded-xl shadow-lg border border-slate-200 mb-6">
              ...
              <button onClick={() => setShowExpertNote(!showExpertNote)} className="mt-4 text-sm text-blue-600 flex items-center gap-2 underline">
                <HelpCircle size={16} /> {showExpertNote ? 'Ocultar Nota de Experto' : 'Ver Nota de Experto'}
              </button>
              {showExpertNote && <p className="mt-2 p-3 bg-amber-50 text-amber-800 rounded-lg text-sm italic">{q.expertNote}</p>}
            </div>
          ))}
          
          <button 
            onClick={() => currentLevel === 1 ? setCurrentLevel(2) : calculateResults()}
            className="w-full bg-slate-900 text-white p-4 rounded-xl font-bold flex items-center justify-center gap-2"
          >
      ...
      {screen === 'report' && results && (
        <div className="max-w-2xl mx-auto bg-white p-8 rounded-2xl shadow-2xl print:shadow-none" id="report">
          ...
          <div className="flex gap-4 print:hidden">
            ...
            <button onClick={() => window.location.reload()} className="flex-1 bg-blue-600 text-white p-3 rounded-lg flex items-center justify-center gap-2"><RefreshCw size={20} /> Reiniciar</button>
          </div>
        </div>
      )}
    </div>
